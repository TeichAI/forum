...
import { Archive, Ban, Inbox, MailOpen, RotateCcw, Shield, Star, Trash2, XCircle } from "lucide-react";
...
export async function MailReader({ viewer, threadId, folder, staffFolder = "inbox" }: { viewer: MailAccessViewer; threadId: string; folder: string; staffFolder?: string }) {
  const participant = await getMailThread(viewer, threadId);
  if (!participant) notFound();
  const collective = participant.isStaffMailbox;
  const staffView = participant.accessContext === "staff";
  const other = staffView ? participant.thread.participants[0]?.user : collective ? null : participant.thread.participants.find((item) => item.userId !== viewer.id)?.user;
  const block = !collective && other ? await db.block.findFirst({ where: { OR: [{ blockerId: viewer.id, blockedId: other.id }, { blockerId: other.id, blockedId: viewer.id }] }, select: { blockerId: true } }) : null;
  const blocked = !collective && (Boolean(block) || participant.thread.participants.some((item) => item.user.status !== "ACTIVE"));
  const unread = isMailUnread(participant);
  const backHref = `/mail?folder=${folder}${folder === "staff" ? `&staffFolder=${staffFolder}` : ""}`;
  return <article className="flex h-full min-h-0 flex-col">
    <MailReadReceipt threadId={threadId} unread={unread} />
    <header className="border-b p-4 sm:p-5" style={{ borderColor: "var(--line)" }}><div className="flex items-start gap-3"><Link href={backHref} className="button button-ghost mobile-only !h-9 !w-9 !p-0" aria-label="Back to mail list">←</Link><div className="min-w-0 flex-1"><h1 className="text-xl font-black leading-tight">{participant.thread.subject}</h1>{other ? <Link href={`/members/${other.id}`} className="mt-2 flex items-center gap-2 text-sm muted"><Avatar src={other.imageUrl} name={other.displayName} className="!h-7 !w-7" /><strong style={{ color: "var(--foreground)" }}>{other.displayName}</strong><span>@{other.username}</span><UserRoleBadge role={other.role} /></Link> : collective ? <div className="mt-2 flex items-center gap-2 text-sm muted"><Shield size={18} aria-hidden /><strong style={{ color: "var(--foreground)" }}>Staff Mailbox</strong></div> : null}</div><div className="flex flex-wrap justify-end gap-1">
      ...
      {!collective && other && <details className="relative"><summary className="button button-ghost !h-9 !w-9 !p-0 list-none" aria-label="Mail safety options">•••</summary><div className="card absolute right-0 top-10 z-20 w-56 p-3"><RateLimitForm action={blockMember}><input type="hidden" name="userId" value={other.id} /><SubmitButton className="button button-danger w-full" pendingLabel="Blocking…"><Ban size={15} /> Block member</SubmitButton></RateLimitForm></div></details>}
    </div></div></header>
    <div className="min-h-0 flex-1 overflow-y-auto p-4 sm:p-6"><div className="space-y-4">{participant.thread.entries.map((entry) => <section key={entry.id} className="rounded-xl border p-4" style={{ borderColor: "var(--line)", background: entry.authorId === viewer.id ? "var(--brand-soft)" : "var(--surface-soft)" }}><header className="mb-3 flex flex-wrap items-center gap-2 text-xs muted"><Avatar src={entry.author.imageUrl} name={entry.author.displayName} className="!h-7 !w-7" /><strong style={{ color: "var(--foreground)" }}>{entry.authorId === viewer.id ? "You" : entry.author.displayName}</strong><span>@{entry.author.username}</span><time className="ml-auto">{format(entry.createdAt, "PPp")}</time>{entry.authorId !== viewer.id && <ReportForm targetType="MAIL_ENTRY" targetId={entry.id} returnTo={`/mail/${threadId}`} />}</header><Markdown>{entry.body}</Markdown></section>)}</div></div>
    <footer className="border-t p-4" style={{ borderColor: "var(--line)" }}>{blocked ? <p className="rounded-xl p-3 text-sm muted" style={{ background: "var(--surface-soft)" }}>You can read this thread, but replies are unavailable because one member blocked the other or the recipient is inactive.</p> : <RateLimitForm action={replyToMail}><input type="hidden" name="threadId" value={threadId} /><MarkdownEditor rows={3} placeholder={`Reply to ${collective && !staffView ? "Staff Mailbox" : other?.displayName ?? "this thread"}…`} /><div className="mt-3 flex justify-end"><SubmitButton pendingLabel="Sending…">Send reply</SubmitButton></div></RateLimitForm>}</footer>
  </article>;
}
