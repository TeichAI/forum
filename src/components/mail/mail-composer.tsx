...
export function MailComposer({ role, initialRecipients = [], draft, uploadsEnabled }: { role: string; initialRecipients?: Recipient[]; draft?: Draft; uploadsEnabled: boolean }) {
  ...
  const sendRateState = sendState.status === "rate_limited" ? sendState : null;
  const saveRateState = saveState.status === "rate_limited" ? saveState : null;
  const { coolingDown: sendCoolingDown, onReady: onSendReady } = useRateLimitCooldown(sendRateState);
  const { coolingDown: saveCoolingDown, onReady: onSaveReady } = useRateLimitCooldown(saveRateState);
  const actionCoolingDown = sendCoolingDown || saveCoolingDown;
  const { coolingDown: uploadCoolingDown, onReady: onUploadReady } = useRateLimitCooldown(uploadLimit);
  ...
  return <form action={sendAction} className="mail-compose-form" aria-busy={sending}>
    ...
    <div className="flex min-h-0 flex-1 flex-col p-4 sm:p-6"><div className="mb-2 flex flex-wrap gap-1" role="toolbar" aria-label="Mail formatting"><FormatButton label="Bold" onClick={() => wrap("**")}><Bold size={15} /></FormatButton><FormatButton label="Italic" onClick={() => wrap("*")}><Italic size={15} /></FormatButton><FormatButton label="Link" onClick={() => wrap("[", "](https://)", "link text")}><LinkIcon size={15} /></FormatButton><FormatButton label="Bulleted list" onClick={() => wrap("- ", "", "list item")}><List size={15} /></FormatButton><FormatButton label="Numbered list" onClick={() => wrap("1. ", "", "list item")}><ListOrdered size={15} /></FormatButton><FormatButton label="Quote" onClick={() => wrap("> ", "", "quote")}><Quote size={15} /></FormatButton>{uploadsEnabled && uploadCoolingDown ? <button type="button" className="button button-ghost !h-8 !px-2" disabled><ImagePlus size={15} /><span className="sr-only">Add inline image</span></button> : uploadsEnabled ? <UploadButton endpoint="mailImageUploader" appearance={{ button: "button button-ghost !h-8 !px-2", allowedContent: "hidden" }} content={{ button: <><ImagePlus size={15} /><span className="sr-only">Add inline image</span></> }} onClientUploadComplete={(files) => { setUploadError(""); setUploadLimit(null); const file = files[0]; if (file?.serverData?.url) setBody((value) => `${value}${value ? "\n\n" : ""}![${file.name}](${file.serverData.url})`); }} onUploadError={(error) => { setUploadError(error.message); setUploadLimit(/too quickly|temporary security check/i.test(error.message) ? {} : null); }} /> : null}</div>{uploadError ? <div className="mb-2 text-xs font-semibold" style={{ color: "var(--danger)" }} role="alert">{uploadError}{uploadLimit ? <div className="mt-1"><RateLimitCountdown trigger={uploadLimit} onReady={onUploadReady} className="text-xs font-semibold" /></div> : null}</div> : null}<textarea ref={textarea} className="min-h-64 flex-1 resize-none bg-transparent text-[0.95rem] leading-7 outline-none" name="body" value={body} onChange={(event) => setBody(event.target.value)} maxLength={50_000} placeholder="Write your mail…" aria-label="Mail body" required /><div className="mt-2 flex justify-between text-xs muted"><span>Markdown formatting · images are inline</span><span>{body.length.toLocaleString()} / 50,000</span></div>{sendState.status === "error" && <p className="mt-2 text-sm font-semibold" style={{ color: "var(--danger)" }} role="alert">{sendState.message}</p>}{sendState.status === "rate_limited" && <div className="mt-2 text-sm font-semibold" style={{ color: "var(--danger)" }} role="alert"><p>{sendState.message}</p><RateLimitCountdown trigger={sendState} onReady={onSendReady} /></div>}</div>
    <footer className="flex flex-wrap items-center gap-2 border-t px-4 py-3 sm:px-6" style={{ borderColor: "var(--line)" }}><button type="submit" className="button button-primary" disabled={sending || actionCoolingDown || !recipients.length || !subject.trim() || !body.trim()}>{sending ? "Sending…" : recipients.length > 1 ? `Send ${recipients.length} private copies` : "Send mail"}</button><button type="button" className="button button-secondary" disabled={actionCoolingDown} onClick={() => void saveAndClose()}>Save & close</button><button type="button" className="button button-ghost" disabled={actionCoolingDown} onClick={() => void discard()}>Discard</button><span className="ml-auto text-xs muted" role="status">{saveState.status === "saved" ? "Draft saved" : saveState.status === "error" || saveState.status === "rate_limited" ? saveState.message : saveState.message ?? "Autosaves after 1 second"}</span>{saveState.status === "rate_limited" ? <RateLimitCountdown trigger={saveState} onReady={onSaveReady} className="text-xs font-semibold" /> : null}</footer>
  </form>;
}
