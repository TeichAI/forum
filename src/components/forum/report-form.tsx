...
export function ReportForm({ targetType, targetId, returnTo }: { targetType: "THREAD" | "REPLY" | "USER" | "MESSAGE"; targetId: string; returnTo: string }) {
  return (
    <details className="relative">
      ...
      <form action={reportContent} className="card absolute right-0 top-7 z-20 w-72 space-y-3 p-4 shadow-xl">
        ...
        <div><label className="label" htmlFor={`reason-${targetId}`}>Reason</label><select className="input" id={`reason-${targetId}`} name="reason"><option>Spam</option><option>Harassment</option><option>Unsafe content</option><option>Off topic</option><option>Other</option></select></div>
