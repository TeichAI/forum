...
const mocks = vi.hoisted(() => ({ viewer: vi.fn(), unread: vi.fn(), mode: vi.fn() }));
...
describe("Header", () => {
  ...
  it.each(["MEMBER", "MODERATOR", "ADMIN"])("renders %s controls and unread state", async (role) => {
    mocks.viewer.mockResolvedValue({ id: "user", role });
    ...
    expect(screen.getByRole("button", { name: "Clerk account" })).toBeInTheDocument();
  });

  it("uses a deterministic account marker in E2E mode", async () => {
    mocks.viewer.mockResolvedValue({ id: "user", role: "MEMBER" });
    mocks.mode.mockReturnValue(true);
    render(await Header());
    expect(screen.getByText("Test user")).toBeInTheDocument();
    expect(screen.queryByText("Clerk account")).not.toBeInTheDocument();
  });
});
