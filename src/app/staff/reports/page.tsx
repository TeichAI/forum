...
export default async function ReportsPage({ searchParams }: { searchParams: Promise<Record<string, string | undefined>> }) {
  ...
  const page = Math.min(normalizedPage, pages);
  const cases = await db.moderationCase.findMany({
    ...
    orderBy: [{ priority: "desc" }, { createdAt: "asc" }], skip: (page - 1) * PAGE_SIZE, take: PAGE_SIZE,
  });
  return (
    <section>
      ...
      {pages > 1 && <nav className="mt-4 flex items-center justify-end gap-2" aria-label="Report pages"><Link className="button button-secondary" aria-disabled={page === 1} href={{ pathname: "/staff/reports", query: { ...params, page: Math.max(1, page - 1) } }}>Previous</Link><span className="text-sm muted">Page {page} of {pages}</span><Link className="button button-secondary" aria-disabled={page === pages} href={{ pathname: "/staff/reports", query: { ...params, page: Math.min(pages, page + 1) } }}>Next</Link></nav>}
    </section>
  );
}
